...
volatile uint16_t command_widx = 0;
volatile uint16_t command_ridx = 0;
...
void printUSART2(char *str, ...) {  /// print text and up to 10 arguments!
  ...
  // va_start(vl, 10);													// always pass the
  // last named parameter to va_start, for compatibility with older compilers
        ...
        switch (str[k + 1]) {
          ...
          case ('f'): {  // float
            uint64_t utmp64 = va_arg(
                vl,
                uint64_t);  // convert double to float representation IEEE 754
            ...
            utmp32 = utmp32 << 3;
            utmp32 |= tmp1;
            if (utmp64 & 0x8000000000000000) utmp32 |= 0x80000000;

            if (utmp64 & 0x4000000000000000) utmp32 |= 0x40000000;
...
void USART2_IRQHandler(void) {
  ...
  if (USART2->SR & (USART_SR_RXNE)) {
    data = USART2->DR;
    if ((data == '\r') || widx >= COMMAND_SIZE) {
      commands[command_widx][widx++] = '\0';

      command_widx = (++command_widx) % COMMANDS_NUMBER;
      widx = 0;
    } else {
      commands[command_widx][widx++] = data;
    }
  }
